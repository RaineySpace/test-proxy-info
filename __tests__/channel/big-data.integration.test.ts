...
describe.skipIf(SKIP_INTEGRATION_TESTS)('BigData 通道集成测试', () => {
  it('应该返回有效的 IP 信息', async () => {
    const result = await testProxyInfo({ channel: TestProxyChannel.BigData });
  ...
  it('返回结果应该包含所有必要字段（timezone 除外）', async () => {
    const result = await testProxyInfo({ channel: TestProxyChannel.BigData });
